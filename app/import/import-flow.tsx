...
function statusFor(step: Step, isPending: boolean, error: string | null): {
  label: string;
  tone: StatusTone;
} {
  if (error) return { label: "Action needs attention", tone: "error" };
  if (isPending) return { label: step === "review" ? "Saving" : "Parsing", tone: "pending" };
...
export function ImportFlow({ existingProfile }: { existingProfile: Resume | null }) {
  ...
  return (
    <div className="flex flex-1 flex-col bg-newsprint">
      ...
      <main className="mx-auto flex w-full max-w-[var(--page-max-width)] flex-1 flex-col px-4 pb-16 md:px-7 lg:pb-20">
        <div className="flex flex-col gap-6 border-b border-rule py-10 md:flex-row md:items-end md:justify-between md:gap-10">
          <div className="max-w-2xl">
            <p className="font-[family-name:var(--font-mono)] text-caption font-medium uppercase tracking-[0.08em] text-ink-muted">
              Import / {step === "upload" ? "Choose file" : step === "review" ? "Review" : "Complete"}
            </p>
            <h1 className="mt-3 font-[family-name:var(--font-display)] text-heading tracking-[-0.02em] text-ink">
              {step === "done"
                ? isReimport
            ...
            <p className="mt-4 max-w-[70ch] font-[family-name:var(--font-body)] text-body leading-relaxed text-ink-muted">
              {step === "done"
                ? isReimport
            ...
            <Card aria-labelledby="upload-title">
              <CardHeader className="bg-paper-sunken">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 max-w-[65ch] font-[family-name:var(--font-body)] text-body leading-relaxed text-ink-muted">
              ...
              <CardContent>
                <form onSubmit={handleUploadSubmit} className="flex flex-col gap-5">
                  <Label htmlFor="resume-file" className="flex flex-col gap-2">
                    Word template
                    <Input
                      id="resume-file"
                      name="file"
          ...
          {step === "review" && parsed && (
            <div className="flex flex-col gap-5">
              {!parsed.isOfficialTemplate && parsed.warning && (
                <Card aria-labelledby="parse-warning-title" className="border-rule bg-paper-sunken">
                  <CardContent className="flex items-start gap-3 p-5">
                    ...
                    <div>
                      <h2 id="parse-warning-title" className="font-[family-name:var(--font-ui)] text-body font-semibold text-ink">
                        Best-effort parse
                      </h2>
                      <p className="mt-1 font-[family-name:var(--font-body)] text-body text-ink-muted">{parsed.warning}</p>
                    </div>
                  </CardContent>
                </Card>
              )}
              ...
              <Card aria-labelledby="parsed-result-title">
                ...
                <CardContent>
                  <p className="font-[family-name:var(--font-display)] text-heading-sm text-ink">
                    ...
                    <div className="mt-5 divide-y divide-rule border-y border-rule">
                      {parsed.resume.sections.map((section) => (
                        <div key={section.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                          <span className="font-[family-name:var(--font-ui)] text-body font-semibold text-ink">{section.title}</span>
                          <span className="font-[family-name:var(--font-mono)] text-caption text-ink-muted">
              ...
              <Card aria-labelledby="review-items-title" className="bg-paper-sunken">
                ...
                <CardContent className="pt-3">
                  {parsed.review.length === 0 ? (
                    <p className="font-[family-name:var(--font-body)] text-body text-ink-muted">
                    ...
                    <ul className="flex flex-col divide-y divide-rule border-y border-rule">
                      {parsed.review.map((item, index) => (
                        <li key={index} className="flex items-start gap-3 py-4">
                          <AlertTriangle className="mt-0.5 size-5 shrink-0 text-editorial" aria-hidden="true" />
                          <div>
                            <p className="font-[family-name:var(--font-ui)] text-body font-semibold text-ink">
                              {REVIEW_KIND_LABEL[item.kind] ?? item.kind}
                            </p>
                            {item.text && <p className="mt-1 font-[family-name:var(--font-body)] text-body text-ink-muted">{item.text}</p>}
                          </div>
                        </li>
                      ))}
                    </ul>
          ...
          {step === "done" && (
            <Card aria-labelledby="saved-title" className="border-ink">
              <CardContent className="flex flex-col items-start gap-5 p-6 md:p-8">
                ...
                <div>
                  <h2 id="saved-title" className="font-[family-name:var(--font-display)] text-heading-sm text-ink">Saved successfully</h2>
                  <p className="mt-2 max-w-[65ch] font-[family-name:var(--font-body)] text-body leading-relaxed text-ink-muted">
                    {isReimport
